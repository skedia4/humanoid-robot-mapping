...
class Map(object):
    def __init__(self):
        self.rate = rospy.Rate(2) ############### Use it to change rospy rate
    ...
    def odo_callback(self, pose_msg):
        ...
        ############################## Note: the following code is for elevation mapping comment if runnig perforance is slow.
        header = pose_msg.header
        pose_w_c = pose_msg.pose
        msg = PoseWithCovarianceStamped()
        msg.header = header
        msg.pose = pose_w_c
        self.pub_3.publish(msg) ######## pose publisher useful for elevation mapping
    ...
    # Extract the values from Q
        ...
        if self.orientation is not None:
            ####### manual rotation conversion slower than scipy
            ########################################################################################
    ...
    def start_cc_raw_pcd(self):
        while not rospy.is_shutdown():
            if self.map_from_raw_pcd is not None:
                ...
                data = self.edt.astype(dtype=[ ('x', np.float32), ('y', np.float32)])
                msg = ros_numpy.msgify(Image, self.cc.data.astype(dtype= np.uint16), "mono16") ####### publishing OCC grid
                #msg = ros_numpy.msgify(Image, self.edt.astype(dtype= np.uint16), "mono16")  ####### publishing EDT
                self.pub_1.publish(msg)
                
                
                point = PointStamped()
                if self.pos_pixel is not None:
                ...
                #################################################################### Uncomment if needs to vizulize the occ grid or EDT directly
                #plt.imshow(self.cc.data, cmap = 'gray', interpolation='nearest')
                ...
                ################################################################ Save NPZ for planning processing
                outfile = 'edt_occ.npz'
...
def map_start():
    ...
    try:
        ######## From The processed map topic \map
        #cc.start_cc()
        
        ######## From the raw point cloud data (currently used)
        cc.start_cc_raw_pcd()
